...
root_path = os.getcwd()
feat_map = ['rms', 'ar', 'iav', 'mav', 'mav1', 'mav2', 'zc', 'ssc', 'wl', 'aac', 'dasdv', 'var', 'tm3',
            'tm4', 'tm5', 'vorder', 'log', 'mnf', 'mdf', 'pkf', 'mnp', 'ttp', 'vcf', 'sm1', 'sm2', 'sm3', 'ssi', ]
...
def training_lda_single_feat(my_clfs, trains, classes, **kw):
    ...
    clf = sklearn.lda.LDA(solver='svd', shrinkage=None, priors=None,
                          n_components=4, store_covariance=False, tol=0.0001)
    for i in range(len(feat_map)):
        ...
        scores = sklearn.cross_validation.cross_val_score(
            clf, trains[i], classes, cv=cv)
        # print "score:", scores.mean()," type of scores: ", type(scores)
        results.append([feat_map[
                       i], "LDA(solver='svd', shrinkage=None, priors=None, n_components=4, store_covariance=False, tol=0.000, cv=5)", scores.mean(), scores.std()])

    clf = sklearn.lda.LDA(solver='lsqr', shrinkage='auto', priors=None,
                          n_components=4, store_covariance=False, tol=0.0001)
    for i in range(len(feat_map)):
        if i != 3 and i != 6 and i != 7 and i != 8:
            continue
        scores = sklearn.cross_validation.cross_val_score(
            clf, trains[i], classes, cv=cv)
        results.append([feat_map[
                       i], "LDA(solver='lsqr', shrinkage=auto, priors=None, n_components=3, store_covariance=False, tol=0.000, cv=5)", scores.mean(), scores.std()])
...
def training_lda_TD4(my_clfs, trains, classes, **kw):
    ...
    while(iteration > 0):
        ...
        X_train, X_test, y_train, y_test = sklearn.cross_validation.train_test_split(
            trains, classes, test_size=0.8, random_state=random.randrange(1, 51))
        scores = sklearn.cross_validation.cross_val_score(
            clf, trains, classes, cv=5)
        results.append(
            ['feat_TD4', "LDA(solver='svd', shrinkage=None, priors=None, n_components=None, store_covariance=False, tol=0.0001)", 1, scores.mean(), scores.std()])

        # proportion is 0.9
        X_train, X_test, y_train, y_test = sklearn.cross_validation.train_test_split(
            trains, classes, test_size=0.2, random_state=random.randrange(1, 51))
        ...
        results.append(['feat_TD4', clf, '0.8+0.9',
